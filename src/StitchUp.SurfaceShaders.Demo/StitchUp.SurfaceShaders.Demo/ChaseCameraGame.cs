...
namespace StitchUp.SurfaceShaders.Demo
{
	/// <summary>
		...
		protected override void LoadContent()
		{
			...
			foreach (ModelMesh mesh in shipModel.Meshes)
				foreach (ModelMeshPart meshPart in mesh.MeshParts)
					meshPart.Effect.Parameters["basic_material2_diffuse_texture"].SetValue(Content.Load<Texture>("ShipDiffuse"));

			groundModel = Content.Load<Model>("Ground");
			foreach (ModelMesh mesh in groundModel.Meshes)
				foreach (ModelMeshPart meshPart in mesh.MeshParts)
					meshPart.Effect.Parameters["basic_material2_diffuse_texture"].SetValue(Content.Load<Texture>("Checker"));
		}
